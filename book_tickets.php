...
if (isset($_POST['submit'])) {
    ...
    // $targetdir="student_image/";

    // if (($imagetype=="image/jpeg") OR ($imagetype="image/png") OR ($imagetype="image/jpg")){
    //     if (move_uploaded_file($imagesync,$targetdir.$imagename)){

    $sql = "INSERT INTO users (sname, oname, date, status, address, phone, gender, height, origin, destination, purpose, passid, passidiss, passidexp, prof, sts, userid, trackid) 
	        VALUES ('$sname','$oname','$date','$status','$address','$phone','$gender','$height','$origin','$destination','$purpose','$passid','$passidiss','$passidexp','$prof','$sts','$userid','$trackid')";

    $query =  mysqli_query($conn, $sql);

    if ($query == TRUE) {
        echo "<script> alert('Record Created Successfully'); window.location='view_booked_tickets.php'; </script>";
    }else {
        echo "Records Not Created";
    }   
// }
//     }else{
//         echo "<script> alert('The Image File is not Supported'); </script>";
//     }
}
?>
	<body>
		...
		<form action="" method="POST">
			...
			<table cellpadding="5">
				...
				<tr>
					<td class="fix_table">
						<input name="sname" placeholder="eg Adeola" required>

						<!-- <input type="text" name="origin" placeholder="From" required> --></td>
					<td class="fix_table">
					<input name="oname" placeholder="eg Tolulope Adisa" required>
					...
					<input type="date" name="date" required>
						...
						<!-- <input type="text" name="origin" placeholder="From" required> --></td>
						<td class="fix_table">
						<input name="status" placeholder="eg Single" required>
			...
			<table cellpadding="5">
				...
				<tr>
					...
					<input type="number" name="phone" required>
						...
						<select name="gender">
						<option value="economy">--Select Gender--</option>
			...
			<table cellpadding="5">
				<tr>
					...
					<td class="fix_table">Tack ID</td>
					<!-- <td class="fix_table">Passport No</td> -->
				</tr>
				<tr>
					<td class="fix_table">
					<input type="date" name="passidiss" required>


						<!-- <input type="text" name="origin" placeholder="From" required> --></td>
					<td class="fix_table">
					<input type="date" name="passidexp" required>
						...
						$ran = "TRC/".$d."/".$ra; 
						?>
						<input type="text" name="trackid" value="<?php echo $ran; ?>" readonly>
				...
				<tr>
						<td class="fix_table">
						<input type="hidden" name="sts" value="0" required>
						<?php $user = $_SESSION['login_user']; ?>
						<input type="hidden" name="userid" value="<?php echo $user; ?>" required>
				</tr>
			</table><br>
